...
void policies_t::inherit_from_env(){
    ...
    if(strcmp(VS_GLOBAL_POLICY,"all")==0){all();}
    ...
    else if(VS_GLOBAL_POLICY==nullptr) {normal();}
    else {vs_log(severety_t::WARNING,nullptr,"Unrecognized VS_GLOBAL_POLICY `%s`, defaulting to `normal`",VS_GLOBAL_POLICY);normal();}

    const char* VS_NETWORKING_POLICY = getenv("VS_NETWORKING_POLICY");
    if(strcmp(VS_NETWORKING_POLICY,"all")==0){networking.all();}
    ...
    if(strcmp(VS_SCRIPTS_POLICY,"all")==0){embedded_scripts.all();}
    ...
    if(strcmp(VS_NATIVE_COMPONENTS_POLICY,"all")==0){allow_native_components=true;}
