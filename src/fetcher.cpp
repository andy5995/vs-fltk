...
std::tuple<resolve_path::reason_t::t,cache::buffer_t, scoped_rpath_t> fetcher(resolve_path& base, resolve_path::from_t from,const char* src, cache::resource_t restype, bool promote, bool preserve){
    auto ret = base(from,src);
    if(ret.first==resolve_path::reason_t::OK){

        //If present, use the one cached.
        {
            auto found = globals::memstorage.get({ret.second.location,0,restype});
            if(found!=nullptr){
        ...
        if(ret.second.type==rpath_type_t::FS){
            auto res = globals::memstorage.fetch_from_fs({ret.second.location,0,restype,promote,preserve});
...
#       ifdef HAS_CURL
            ...
            auto res = globals::memstorage.fetch_from_http({ret.second.as_string(),0,restype,promote,preserve});
            if(res==globals::memstorage.end()){
                return {resolve_path::reason_t::NOT_FOUND,{nullptr,0},ret.second};
            }
            else{
            ...
            auto res = globals::memstorage.fetch_from_https({ret.second.as_string(),0,restype,promote,preserve});
            if(res==globals::memstorage.end()){
                return {resolve_path::reason_t::NOT_FOUND,{nullptr,0},ret.second};
            }
            else{
